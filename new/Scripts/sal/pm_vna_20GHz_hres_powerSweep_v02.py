...
#Set up data
filename = 'pm_vna_20GHz_hres_powerSweep_v02'
data = qt.Data(name=filename)
data.add_coordinate('Frequency [MHz]',size=sweep_points)
data.add_coordinate('Power [dBm]',size=power_points)
data.add_value('S21 [even]')
data.add_value('S21 [odd]')
#data.add_value('S21 []')
#data.add_value('Summed trace ')

data.create_file()
data.copy_file('pm_vna_20GHz_hres_powerSweep_v02.py')
...
for i in power_list:
    ...
    while (run_index<exp_run):
        ...
        data.add_data_point(flist,dummy_power,magnitude_list,phase_list)
