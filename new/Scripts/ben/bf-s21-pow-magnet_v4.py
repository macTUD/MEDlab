...
med.set_device('Device with Graphite JLCs (smal-flake (dev A))')
med.set_setup('BF')
...
filename = 'S21_pow_magnet_graphJLC'
data = qt.Data(name=filename)
data.add_coordinate('Frequency (Hz)')
data.add_coordinate('Magnet [10mA/1V]')
data.add_coordinate('Power [dBm]')
data.add_value('S21 [Absolute] ')
data.add_value('S21 [Phase]')
data.create_file()
data.copy_file('bf-s21-pow-magnet_v4.py')
...
f1_stop= 7.8*GHz
f1_pts = 2001
f1_bw = 30*kHz #float(f1_stop-f1_start)/float(f1_pts) #500e3#1e9/180e3 #expected q factor

#dim 2
gate_start = -1         #gate setting x1
gate_stop = 1           #gate setting x1
gate_multi = 1          #enter here the S1d multiplier(1 or 4)
gate_bitstep = 3      #number of bits to jump per step 1bit = 0.304mV * (S1d multiplier)
#(600 = 0.2V steps)
