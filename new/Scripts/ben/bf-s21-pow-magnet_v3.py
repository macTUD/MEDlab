...
filename = 'S21_pow_Vg_graphJLC'
data = qt.Data(name=filename)
data.add_coordinate('Frequency (Hz)')
data.add_coordinate('Magnet [20mA/1V]')
data.add_coordinate('Power [dBm]')
data.add_value('S21 (Mlog) [dBm]]')
data.add_value('S21 [Phase]')
data.create_file()
data.copy_file('bf-s21-pow-magnet_v3.py')
...
#dim 2
gate_start = -2         #gate setting x1
gate_stop = 0           #gate setting x1
gate_multi = 1          #enter here the S1d multiplier(1 or 4)
gate_bitstep = 30      #number of bits to jump per step 1bit = 0.304mV * (S1d multiplier)
#(600 = 0.2V steps)
...
for f1_power in pow_list:
    ...
    for bit_gate in gbit_list:
        ...
        print f1_power,'Pow [dBm] ', gate,' [V] estimated ready at:', localtime(tstart+exp_time)[2],'-',localtime(tstart+exp_time)[1],'-',localtime(tstart+exp_time)[0],' ',localtime(tstart+exp_time)[3], ':', localtime(tstart+exp_time)[4], ':', localtime(tstart+exp_time)[5]
        ...
        data.new_block()
        spyview_process(data,len(f_list),f1_start,f1_stop,len(gbit_list),gate_start,gate_stop, pow_start,f1_power)
