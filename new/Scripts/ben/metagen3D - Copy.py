def spyview_process(data=0,minval=0,maxval=0,minblockval=0,maxblockval=0,outermostblockval=0,reset=False,_outermostblockvals=[]):
    '''
    ...
    if reset==True:
        ...
        metafile.write('#outer most loop\n%s\n%s\n%s\n%s\n'%(len(_outermostblockvals), min(_outermostblockvals), max(_outermostblockvals),data.get_dimension_name(2)))
