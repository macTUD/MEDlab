...
data.create_file()
data.copy_file('bf-s21-pow-vg_v5.py')
...
#dim 2
gate_start = -8         #gate setting x1
gate_stop = 8           #gate setting x1
...
for f1_power in pow_list:
    ...
    for bit_gate in gbit_list:
        ...
        print f1_power,'Pow [dBm] ', gate,' [V] estimated ready at:', localtime(now_time+exp_time)[2],'-',localtime(now_time+exp_time)[1],'-',localtime(now_time+exp_time)[0],' ',localtime(now_time+exp_time)[3], ':', localtime(now_time+exp_time)[4], ':', localtime(now_time+exp_time)[5]
